...
import { User, MessageReaction, ClientUser, Guild, TextChannel } from 'discord.js'
...
export default class extends Event {
  ...
  async run(packet: { [key: string]: any }) {
    ...
    member.send({
      embed: {
        "title": "```⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀МЕНЮ СЕРВЕРА⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀```",
        "description": "Чтобы получить информацию о нашем сервере, необходимо нажать на одну из реакций ниже или воспользоваться командами.\n\n⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀**!welcome**⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀\n```ini\n[позволит вам узнать информацию о голосовых каналах, ролях и способах их получения.]```\n⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀**!commands**⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀\n```css\n[покажет вам возможности нашего бота, которые значительно украсят и упростят жизнь.]```\n⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀**!rules**⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀\n```fix\n откроет для вас правила нашего сервера.```",
        "color": 3092790,
        "footer": {
          "text": "Для повторного вызова меню в будущем, необходимо использовать команду !info."
        },
        "image": { url: "https://cdn.discordapp.com/attachments/724573511569637407/774740843457937448/menu.gif" }
      }
    }).then(msg => {
      msg.delete({ timeout: 1.8e6 })
      ...
      msg.react(config.emojis.arrowBackwardMenu).then(() => {
        ...
        const backwardsFilter = (reaction: MessageReaction, user: User) => reaction.emoji.id === config.emojis.arrowBackwardMenu && user.id === member.id;
        const forwardsFilter = (reaction: MessageReaction, user: User) => reaction.emoji.id === config.emojis.arrowForwardMenu && user.id === member.id;
        const backwards = msg.createReactionCollector(backwardsFilter, { time: 1.8e6 });
        const forwards = msg.createReactionCollector(forwardsFilter, { time: 1.8e6 });
        const rulesf = (reaction: MessageReaction, user: User) => reaction.emoji.id === config.emojis.rulesMenu && user.id === member.id;
        const rules = msg.createReactionCollector(rulesf, { time: 1.8e6 });
        const welcomef = (reaction: MessageReaction, user: User) => reaction.emoji.id === config.emojis.welcomeMenu && user.id === member.id;
        const welcome = msg.createReactionCollector(welcomef, { time: 1.8e6 });
        const commandf = (reaction: MessageReaction, user: User) => reaction.emoji.id === config.emojis.commandsMenu && user.id === member.id;
        const command = msg.createReactionCollector(commandf, { time: 1.8e6 });
        const backetf = (reaction: MessageReaction, user: User) => reaction.emoji.name === '🗑️' && user.id === member.id;
        const backet = msg.createReactionCollector(backetf, { time: 1.8e6 });
