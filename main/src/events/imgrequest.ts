...
import { IimgRequest, ImgRequest, User } from '../utils/db'
...
const allowedRoles = [config.ids.roles.owner, config.ids.roles.orion, config.ids.roles.sirius, config.ids.roles.astral]
async function ManageReaction(reaction: MessageReaction, user: Discord.User, post: IimgRequest) {
    const guild = reaction.message.guild;
    switch (reaction.emoji.id) {
        case verifyEmojis[0]:
            if (post) {
                const msgEmbed: { [key: string]: any } = reaction.message.embeds[0] || {}
                ...
                const channel = guild?.channels.cache.get(config.postSend[post.type]) as TextChannel
                ...
                const userDB = await User.getOne({ userID: member?.id });
                ...
                member?.send(new MessageEmbed({
                    ...
                    color: 3092790,
                    footer: {
                        text: `${member.user.tag || member.user.username} • ты получил(а) ${reward} золота`,
                ...
                message.edit(null, {
                    embed: {
                        ...msgEmbed,
                        footer: {
                            text: `Предложение принято пользовтелем ${user.tag || user.username}`
                        }
                    }
                })
...
export class ImgRequestVerify extends Event {
    ...
    run(packet: { [key: string]: any }) {
        ...
        const channel = this.client.channels.cache.get(packet.d.channel_id) as TextChannel;
        channel.messages.fetch(packet.d.message_id).then(async (message: Message) => {
            ...
            if (packet.t === 'MESSAGE_REACTION_ADD') {
                ...
                if (verifyEmojis.some(e => e == reaction.emoji.id) && allowedRoles.some(r => staff?.roles.cache.has(r))) {
                    const post = await ImgRequest.findOne({ msgID: reaction.message.id })
                    if (post)
                        await ManageReaction(reaction, user, post)
                }
            }
