...
export class PNameCommand extends Command {
  ...
  async execute(message: Message, args: string[], { member }: CommandParams) {
    ...
    if (privateNameChangeTimeout.has(member.id)) {
      const difference = privateNameChangeTimeout.get(member.id)! + config.meta.privateNameInterval - now
      if (difference > 0) {
        message.channel
          .send({
            embed: {
              ...
              description: `Приходи через **${Util.parseFilteredFullTimeArray(difference)[0]}**`
            }
          })
          .then(msg => msg.delete({ timeout: config.meta.errorMsgDeletion }))
          .catch(() => { })
    ...
    channel.edit({ name: name }).then(() => { return message.reply(` вы успешно изменили название канала на \`${name}\``) })
...
export class PLimitCommand extends Command {
  ...
  async execute(message: Message, args: string[], { member }: CommandParams) {
    const channel = member.voice.channel as VoiceChannel
    if (!Util.validatePrivateroom(member, channel)) return
    const limit = parseInt((args[0] || '').replace(/\D/g, ''))
    ...
    if (privateLimitChangeTimeout.has(member.id)) {
      const difference = privateLimitChangeTimeout.get(member.id)! + config.meta.privateLimitInterval - now
      if (difference > 0) {
        message.channel
          .send({
            embed: {
    ...
    channel.edit({ userLimit: limit }).then(() => { return message.reply(` вы успешно изменили количество слотов в канале на \`${limit}\``) })
  }
}
...
export class POwnerTransferCommand extends Command {
  ...
  async execute(message: Message, args: string[], { member }: CommandParams) {
    ...
    if (privateOwnerTransferTimeout.has(member.id)) {
      const difference = privateOwnerTransferTimeout.get(member.id)! + config.meta.privateOwnerTransferInterval - now
      if (difference > 0) {
        message.channel
          .send({
            embed: {
