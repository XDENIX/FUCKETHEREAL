          ...
          .send({
            embed: {
              ...
              description: [
                ...
                '',
                `Приходи через **${Util.parseFilteredFullTimeArray(difference)[0]
                }**, чтобы получить ${Util.resolveEmoji(config.meta.emojis.cy).trim()}`
              ].join('\n')
            }
          })
        ...
        .send({
          embed: {
            ...
            description: [
              ...
              '',
              `Через **${Util.parseFilteredFullTimeArray(config.meta.timelyInterval)[0]
