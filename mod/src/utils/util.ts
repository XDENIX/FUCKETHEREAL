...
export function checkModerators() {
  const daily = async () => {
    const now = Date.now()
    await [...voiceMembers.entries()].map(e => {
      voiceMembers.set(e[0], now)
      const msTime = now - e[1]
      const time = Math.floor(msTime / 1e3)
      return Moderator.getOne({ user_id: e[0] }).then(mod => {
  ...
  new CronJob('0 0 * * *', daily, null, true, config.meta.defaultTimezone)
  new CronJob('0 0 * * 1', weekly, null, true, config.meta.defaultTimezone)
}
